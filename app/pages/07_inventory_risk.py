...
if not risk_df.empty:
    ...
    with f_col2:
        if "risk_level" in df.columns:
            ...
            existing_risks = [r for r in all_risks if r in df["risk_level"].unique()]
            selected_risks = st.multiselect("Filter Risk Level", options=existing_risks, default=[])
            if selected_risks:
                df = df[df["risk_level"].isin(selected_risks)]
                
    with f_col3:
        if "sku_name" in df.columns:
            search_query = st.text_input("Search Product", placeholder="e.g. Headphones")
            if search_query:
    ...
    if not df.empty and "risk_level" in df.columns:
        critical_c = len(df[df["risk_level"] == "Critical"])
        high_c = len(df[df["risk_level"] == "High"])
        
        if critical_c > 0:
            st.error(f"🚨 **Immediate Action Required**: Expedite purchase orders for **{critical_c} Critical Risk** items to prevent imminent stockouts.")
        if high_c > 0:
            st.warning(f"⚠️ **Action Needed**: Review reorder quantities and supplier lead times for **{high_c} High Risk** items.")
        if critical_c == 0 and high_c == 0:
            st.success("✅ No critical or high risk items found in the current view.")
    else:
        st.info("No actionable data available.")
    ...
    avg_score = df["risk_score"].mean() if "risk_score" in df.columns and not df.empty else 0
    ...
    with col2:
        st.subheader("Top Highest Risk Items")
    ...
    with col3:
        ...
        if "category" in df.columns and "risk_level" in df.columns and not df.empty:
    ...
    if "risk_level" in df.columns:
        display_cols = [c for c in ["sku_id", "sku_name", "category", "risk_level", "risk_score"] if c in df.columns]
        if not df.empty:
            table_df = df[display_cols].sort_values("risk_score", ascending=False)
            ...
            st.success("No items match your filter criteria.")
    else:
